...
namespace WorkerRoleQueueSample
{
    public class SeekUResolver : IDependencyResolver
    {
        ...
        public void Register<T, TK>()
            ...
            where TK : T
        {
            _container.Configure(x => x.For<T>().Transient().Use<TK>());
        }
        ...
        public void Register<T, TK>(Action<TK> configurationAction)
            ...
            where TK : T
        {
            _container.Configure(x => x.For<T>().Transient().Use<TK>().OnCreation(configurationAction));
        }

        public void Register<T>(T instance)
        {
            _container.Configure(x => x.For<T>().Transient().Use(instance));
        }
